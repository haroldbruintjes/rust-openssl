...
impl MdCtxRef {
    ...
    /// Copies the EVP_MD_CTX of `source` into `self`.
    ///
    /// This can be used to efficiently digest different data with the same prefix.
    #[corresponds(EVP_MD_CTX_copy_ex)]
    #[inline]
    pub fn clone_from(&mut self, source: &Self) -> Result<(), ErrorStack> {
        unsafe { cvt(EVP_MD_CTX_copy_ex(self.as_ptr(), source.as_ptr()))? };
        Ok(())
    }
}
...
mod test {
    ...
    fn verify_md_ctx_copy() {
        ...
        ctx_there.clone_from(&ctx_world).unwrap();
